...
class PasteModelTest(TestCase):
    ...
    def test_set_delete_time(self):
        ...
        self.paste.save()
        expected_delete_time = (self.paste.created_at
                                + timezone.timedelta(days=1))
        self.assertAlmostEqual(
            self.paste.delete_at,
...
class CommentModelTest(TestCase):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            content="This is a test comment.", paste=self.paste, user=self.user
        )

    def test_string_representation(self):
        expected_string = (
            f"Comment by {self.comment.user.username} "
            f"on {self.comment.paste.title}"
        )
